...
THREE_PEOPLE_FILE = "/root/autodl-tmp/coco_controlnet/1_3people.txt"

with open(THREE_PEOPLE_FILE, "r") as f:
    S = f.read().replace(",", "")
    O = S.split()
...
WRITE_JSON_PATH = "/root/autodl-tmp/coco_controlnet/prompt_3people.json"
...
if __name__ == "__main__":
    ...
    with open(WRITE_JSON_PATH, "w") as f:
        for index in same_index:
            index = str(index).zfill(12)
            txt_path = os.path.join(COCO_TXT_DIR_PATH, index + ".txt")
            txt = open(os.path.join(COCO_TXT_DIR_PATH, txt_path), "r").read()
            hq_img = os.path.join("hq", index + ".jpg")
            ori_img = os.path.join("ori_img", index + ".jpg")
            data = {"source": hq_img, "target": ori_img, "prompt": txt}
