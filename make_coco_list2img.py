...
same_index = list(range(100000000001, 100000001001))
...
with torch.no_grad():
    for i in range(len(same_index)):
        ...
        image_path = os.path.join(SAVE_IMAGE_HQ_PATH, file)
        detected_map_path = os.path.join(SAVE_IMAGE_HQ_PATH, file)
        ...
        for i in range(num_samples):
            ...
            cv2.imwrite(os.path.join(SAVE_IMG_PATH, file), new_img[:, :, [2, 1, 0]])
